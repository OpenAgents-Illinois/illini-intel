...
def _get_json(path: str, timeout: float = 30.0) -> dict[str, Any]:
    response = httpx.get(f"{ESPN_BASE_URL}{path}", timeout=timeout)
...
def fetch_schedule(team_id: str, timeout: float = 30.0) -> dict[str, Any]:
    return _get_json(f"/teams/{team_id}/schedule", timeout=timeout)
